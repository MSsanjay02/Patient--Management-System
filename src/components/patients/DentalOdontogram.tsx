...
> = {
  ...
  Decayed: { bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-200', label: 'Caries / Decayed' },
  Filled: { bg: 'bg-sky-50', text: 'text-sky-700', border: 'border-sky-200', label: 'Composite / Amalgam Fill' },
  Missing: { bg: 'bg-slate-100', text: 'text-slate-400', border: 'border-slate-300', label: 'Missing / Extracted' },
  Crown: { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', label: 'Zirconia Crown' },
  RootCanal: { bg: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-200', label: 'Endodontic RCT' },
  Implant: { bg: 'bg-cyan-50', text: 'text-cyan-700', border: 'border-cyan-200', label: 'Dental Implant' },
};
  ...
  const renderToothCard = (toothNum: number) => {
    ...
    return (
      <button
        ...
        className={`w-11 h-16 rounded-xl border flex flex-col items-center justify-between p-1.5 transition-all ${style.bg} ${style.border} hover:border-sky-500 hover:scale-105 shadow-xs relative group`}
        title={`Tooth #${toothNum}: ${style.label} ${condition.notes ? `\nNotes: ${condition.notes}` : ''}`}
      >
        <span className="text-[10px] font-bold text-slate-500">#{toothNum}</span>
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Universal Numbering System (Teeth 1-32) • Real-time clinical condition logging</p>
        </div>
        {!readOnly && (
          <span className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full flex items-center space-x-1.5 self-start sm:self-auto">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        {(Object.keys(conditionStyles) as Array<ToothCondition['status']>).map((st) => (
          <div
            key={st}
            className={`px-3 py-1 rounded-lg border flex items-center space-x-1.5 text-xs font-bold ${conditionStyles[st].bg} ${conditionStyles[st].border} ${conditionStyles[st].text}`}
          >
      ...
      <div className="space-y-6 bg-slate-50 p-5 rounded-2xl border border-slate-200">
        ...
        <div className="space-y-2">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider text-center flex items-center justify-center space-x-2">
            <span>Upper Right Quadrant</span>
            <span className="text-slate-300">|</span>
            <span className="text-sky-700 font-extrabold">MAXILLARY ARCH (Teeth 1 - 16)</span>
            <span className="text-slate-300">|</span>
            <span>Upper Left Quadrant</span>
          </div>
          <div className="flex justify-center gap-2 flex-wrap">
            <div className="flex gap-1.5 border-r border-slate-200 pr-3">{upperRightQuadrant.map(renderToothCard)}</div>
        ...
        <div className="space-y-2">
          <div className="flex justify-center gap-2 flex-wrap">
            <div className="flex gap-1.5 border-r border-slate-200 pr-3">{lowerRightQuadrant.map(renderToothCard)}</div>
            <div className="flex gap-1.5 pl-3">{lowerLeftQuadrant.map(renderToothCard)}</div>
          </div>
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider text-center flex items-center justify-center space-x-2 pt-2">
            <span>Lower Right Quadrant</span>
            <span className="text-slate-300">|</span>
            <span className="text-teal-700 font-extrabold">MANDIBULAR ARCH (Teeth 17 - 32)</span>
            <span className="text-slate-300">|</span>
      ...
      {selectedTooth && !readOnly && (
        <div className="p-5 bg-white border border-slate-300 rounded-2xl space-y-4 shadow-xl">
            ...
            {(Object.keys(conditionStyles) as Array<ToothCondition['status']>).map((st) => (
              <button
                ...
                className={`px-3 py-2 rounded-xl border text-xs font-bold transition-all ${
                  status === st
                    ? `${conditionStyles[st].bg} ${conditionStyles[st].border} ${conditionStyles[st].text} ring-2 ring-sky-500`
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-sky-600"
          />
